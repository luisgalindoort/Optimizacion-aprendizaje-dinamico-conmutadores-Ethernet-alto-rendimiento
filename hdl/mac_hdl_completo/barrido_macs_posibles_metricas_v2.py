"""
Ejecuta el modelo con cache y el modelo base para distintos espacios muestrales de direcciones MACs.
"""
...
OUTPUT_CSV = "barrido_macs_posibles_metricas_hdl_v2.csv"
OUTPUT_DIR = "graficas_macs_posibles_metricas_hdl_v2"
...
#Ajusta datos  para el guardarlos en CSV.
def metric_row(
    *,
    bitrate_bps: int,
    alpha: float,
...
# Ejecuta modelo con cache y base para un punto concreto del barrido.
def run_case(
    script_dir: Path,
    *,
    bitrate_bps: int,
...
#Convierte un campo del CSV a float.
def value(row: dict, key: str) -> float:
    text = row[key]
    return float(text) if text != "" else float("nan")


#Genera una figura para una metrica del barrido.
def plot_metric(rows: list[dict], metric: str, output_dir: Path) -> None:
    ...
    for i, alpha in enumerate(alphas):
        for j, bitrate in enumerate(bitrates):
            ...
            ax.set_title(f"{bitrate:g} Gbps | alpha={alpha:g}")
    ...
    fig.savefig(output_dir / f"barrido_hdl_{metric}.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
...
#Funcion principal del barrido,lee argumentos, ejecuta el barrido y guarda resultados.
def main() -> None:
